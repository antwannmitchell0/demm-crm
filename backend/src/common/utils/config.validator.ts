export function validateEnvironmentConfig() {
  ...
  if (insecureDefaults.includes(jwtSecret)) {
    console.error('❌ FATAL CONFIGURATION ERROR: JWT_SECRET uses an insecure default key. Refusing to start.');
    process.exit(1);
  }

  if (jwtSecret.length < 32) {
    console.error('❌ FATAL CONFIGURATION ERROR: JWT_SECRET must be at least 32 characters long for production security.');
    process.exit(1);
  }
