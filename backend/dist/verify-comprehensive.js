...
const taskService = new task_service_1.TaskService(prisma);
...
async function main() {
    ...
    await prisma.membership.create({ data: { userId: userA.id, organizationId: orgA.id, workspaceId: wsA.id, role: client_1.Role.ORG_OWNER, permissions: ['*'] } });
    ...
    await prisma.membership.create({ data: { userId: userB.id, organizationId: orgB.id, workspaceId: wsB.id, role: client_1.Role.USER, permissions: ['contact:read'] } });
...
//# sourceMappingURL=verify-comprehensive.js.map